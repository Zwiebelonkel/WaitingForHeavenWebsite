  ...
  if (window.gsap && window.ScrollTrigger) {
    ...
    $$('.reveal').forEach((el) => {
      gsap.fromTo(el,
        ...
        { y: 0, opacity: 1, filter: 'blur(0px)', duration: 1.05, ease: 'power3.out', scrollTrigger: { trigger: el, start: 'top 82%' } }
      );
    });
...
function initMagneticButtons() {
  $$('.magnetic').forEach(btn => {
    btn.addEventListener('mousemove', e => {
      ...
      btn.style.transform = `translate(${x * .10}px, ${y * .14}px) scale(1.02)`;
    });
...
function initVerdicts() {
  ...
  $$('.verdict').forEach(button => {
    button.addEventListener('click', () => {
      ...
      if (window.anime) {
        anime({ targets: panel, translateY: [18, 0], opacity: [.55, 1], duration: 520, easing: 'easeOutExpo' });
      }
    });
  });
}
...
function initFog() {
  ...
  const resize = () => {
    ...
    particles = Array.from({ length: Math.min(90, Math.floor(innerWidth / 18)) }, () => ({
      ...
      vy: (Math.random() - .5) * .10 * devicePixelRatio,
